...
class CustomPersonWidget extends StatefulWidget {
  const CustomPersonWidget({Key? key}) : super(key: key);
...
class CartBody extends StatelessWidget {
  ...
  Container noItemContainer() {
    return Container(
      child: Text(
        "No more Items left in cart",
        style: TextStyle(
            fontSize: 20, fontWeight: FontWeight.w600, color: Colors.grey[20]),
      ),
    );
  }
...
class CartListItem extends StatelessWidget {
  CartListItem({Key? key, required this.foodItem}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return DragTarget<FoodItem>(
      ...
      onAccept: (FoodItem foodItem){
        listBloc.removeToList(foodItem);
        colorBloc.setColor(Colors.white);
      },
      onLeave: (FoodItem? foodItems){
        colorBloc.setColor(Colors.white);
      },
